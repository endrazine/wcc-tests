...
RUN apt-get install -y clang libbfd-dev uthash-dev libelf-dev libcapstone-dev  libreadline-dev libiberty-dev libgsl-dev build-essential git debootstrap file cargo
RUN apt-get install -y libbpf1 libtirpc-dev

# Build wcc & install it
RUN git clone https://github.com/endrazine/wcc.git && cd wcc && git checkout d2e5cff27140bfad5123ee719162920113cbb2d6 && git submodule init && git submodule update && make && make install
...
RUN apt-get install -y libdbus-1-3 libdevmapper1.02.1 libestr0 libglib2.0-0 libglib2.0-dev libkmod2 libmnl0 libnewt0.52 libpopt0 sudo systemd libxtables12 libfastjson-dev
